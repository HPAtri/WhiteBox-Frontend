<template>
    ...
    <el-descriptions-item label="个人简介">{{profile}}</el-descriptions-item>
      ...
      <el-tab-pane label="发帖"><div class="tag" v-for="(item,index) in myArticles" :key="index">
          <h6 class="game-name">{{item.title}}</h3>
          <img :src="item.pic" class="game-img">
      </div></el-tab-pane>
...
<script>
  ...
  export default {
    data() {
      return {
        userList:[],
        myArticles:[{title:"xxx",pic:"https://cdn.hk01.com/di/media/images/dw/20210720/494233049202954240095127.jpeg/Gtaq3XCs2uf6M9GekdCXNBW9HVM_YoBE0mHODdJhzg0?v=w800"}],
        colectArticles:[],
        icon: "",
        nickname: "",
        phone_number:"",
