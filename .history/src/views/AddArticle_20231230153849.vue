      ...
      methods: {
        ...
        initBlog(){
          ...
          this.getRequest('/blog/getByBlogId?id=' + this.id).then(resp=>{
              ...
              that.editForm = resp.obj
              if(resp.obj.published = true){
                that.editForm.published= "公开";
              }
              else{
                that.editForm.published= "私密";
              }
              that.editForm.type_id = resp.obj.typeId + ''
              that.editForm.first_picture = resp.obj.firstPicture

              that.btags = resp.obj.tblogTags
              that.btags.forEach(element => {
                that.tblogTags.push(element.ttag.name + '');                
              });
              that.editForm.tags = that.tblogTags
              console.log(that.editForm.tags)
